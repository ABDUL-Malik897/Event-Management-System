...
const TicketDetails = () => {
    ...
    const downloadTicket = (ticket) => {
        ...
        doc.text(`Venue : ${booking.event.venue}`, 20, 60);
        doc.text(`Seat : ${ticket.seatNumber || "Not Assigned"}`, 20, 70);
        ...
        doc.text(`Amount Paid : Rs. ${booking.totalAmount}`, 20, 110);

        if (booking.razorpayPaymentId) {
            doc.text(`Payment ID : ${booking.razorpayPaymentId}`, 20, 120);
        }
        doc.addImage(ticket.qrCode, "PNG", 20, 135, 60, 60);
        doc.save(`${booking.event.title}-${ticket.seatNumber || ticket.ticketId}.pdf`);
        toast.success("Ticket PDF downloaded successfully");
        } catch (error) {
        console.error("Ticket PDF Download Error:", error);
        toast.error("Unable to download ticket PDF");
        }
    };
    ...
    if (loading) {
        ...
        <div className="ticket-details-loading">
            ...
            <p>Please wait while we fetch your booking.</p>
        </div>
        );
    }
    ...
    if (!booking) {
        ...
        <div className="ticket-not-found">
            <div className="ticket-not-found-icon">🎫</div>
    ...
    const currentTickets = booking.tickets.slice(
        indexOfFirstTicket,
        indexOfLastTicket,
    );
    ...
    return (
        ...
        <section className="ticket-event-banner">
            <img
                src={booking.event.banner || "https://via.placeholder.com/1000x400?text=Event+Banner"}
                alt={booking.event.title}
            />
            <div className="ticket-banner-overlay">
            <span className="ticket-booking-badge">CONFIRMED BOOKING</span>
        ...
        <section className="ticket-booking-summary">
            ...
            <div>
                ...
                <h2>Your Event Booking</h2>
            </div>
            <div
                className={`ticket-payment-status ${
            ...
            <div className="ticket-info-item">
                <div className="ticket-info-icon">📅</div>
                <div>
            ...
            <div className="ticket-info-item">
                <div className="ticket-info-icon">🎟️</div>
                <div>
            ...
            <div className="ticket-payment-details">
                ...
                <strong>{booking.razorpayPaymentId || "Not Available"}</strong>
                </div>
                <div className="ticket-payment-row">
                <span>Order ID</span>
                <strong>{booking.razorpayOrderId || "Not Available"}</strong>
                </div>
            </div>
            )}
        </section>
        <section className="tickets-section">
            ...
            <h2>Your Tickets</h2>
            <p>
                Show the QR code at the event entrance for verification and
                check-in.
            </p>
            </div>
            <div className="tickets-grid">
            {currentTickets.map((ticket, index) => {
                const actualIndex = indexOfFirstTicket + index;
                return (
                <div
                    key={ticket.ticketId || actualIndex}
                    className="digital-ticket"
                >
                    <div className="digital-ticket-header">
                    <div>
                        <span>EVENTHUB TICKET</span>
                        <h3>Ticket #{actualIndex + 1}</h3>
                    </div>
                    <div
                        className={`ticket-checkin-status ${
                    ...
                    <div className="ticket-qr-wrapper">
                        <img
                        src={ticket.qrCode}
                        alt={`QR Code for Ticket ${actualIndex + 1}`}
                        />
                    </div>
                    ...
                    <div>
                        <span>Seat</span>
                        <strong>{ticket.seatNumber || "Not Assigned"}</strong>
                    </div>
            ...
            <div className="ticket-pagination">
                ...
                {Array.from(
                    ...
                    type="button"
                    key={page}
                    className={currentPage === page ? "ticket-page-active" : ""}
        ...
        <section className="ticket-important-note">
            <div className="ticket-note-icon">💡</div>
            <div>
            <h3>Important Information</h3>
            <p>
                Keep your QR ticket safe and do not share it with others. Each
                ticket can only be checked in once at the event entrance.
            </p>
            </div>
        </section>
        </div>
